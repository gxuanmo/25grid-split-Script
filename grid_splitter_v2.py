...
def auto_detect_grid_dimensions(img_path, min_cells=4, max_cells=100):
    ...
    for rows, cols in common_configs:
        ...
        elif abs(score - best_score) <= 0.01 and lines_found > best_lines_found:
            # 分数相同时，选择找到更多线条的配置
            best_config = (rows, cols)
            best_lines_found = lines_found
...
def split_grid_clean(image_path, output_folder, rows, cols, auto_detect=True):
    ...
    rows_info = []
    for i in range(rows):
        y1 = h_positions[i] + margin if i > 0 else h_positions[i]
        y2 = h_positions[i+1] - margin if i < rows - 1 else h_positions[i+1]
        rows_info.append((max(0, y1), max(0, y2)))
    
    cols_info = []
    for j in range(cols):
        x1 = v_positions[j] + margin if j > 0 else v_positions[j]
        x2 = v_positions[j+1] - margin if j < cols - 1 else v_positions[j+1]
    ...
    for i, (y1_orig, y2_orig) in enumerate(rows_info):
        for j, (x1_orig, x2_orig) in enumerate(cols_info):
            ...
            # 从格子中心提取 target_w x target_h 的区域
            center_x = (x1_orig + x2_orig) // 2
            center_y = (y1_orig + y2_orig) // 2
            
            # 确保中心提取不碰到边缘
            x1 = max(x1_orig, center_x - target_w // 2)
            y1 = max(y1_orig, center_y - target_h // 2)
            x2 = x1 + target_w
            y2 = y1 + target_h
